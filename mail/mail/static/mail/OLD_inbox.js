...
function compose_email() {
  ...
  // if (document.querySelector('#view-email')) {
  ...
  //   // e.preventDefault(); // previene correr al 'inbox' immediatamente despues de 'submit'
  //   post_msg();
  // };
}
...
function load_mailbox(mailbox) {
  ...
  .then(emails => {
    ...
    // OPCION 3 (ver al final)
    for (const email in emails) {
      list_emails(emails[email], mailbox);
    }
    // OPCION 2 igual a arriba pero sin mailbox
    // for (const email in emails) {
    //   list_emails(emails[email]);
    // }
  });
